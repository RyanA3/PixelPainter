...
import com.felnstaren.painter.listeners.ExportButtonListener;
...
public class PixelPainter extends AbstractApp {
	...
	public void init(AppContainer ac) {
		...
		this.popman = new PopupManager(bhandler);
		...
		ehandler.addListener(new ExportButtonListener(canvas, popman, bhandler));
		...
		bhandler.addButton(new Button(ac.getWidth() - 50, 2, 14, 14, "export_button", new Image("/resources/icons/export.png")));
		...
		//popman.addPopup(new PopUp("Save", 50, 50, 100, 70));
	}

	public void update(AppContainer ac, float delta_time) {
		bhandler.update(ac);
		popman.update(ac);
		brush.update(ac, cpaint, canvas, popman);
	...
	public void render(AppContainer ac, Renderer renderer) {
		...
		brush.renderEdit(renderer);
		hud.render(renderer, ac);
		bhandler.render(renderer);
		popman.render(renderer);
		brush.renderCursor(renderer);
